...
RSpec.describe ApplicationView do
  describe "#error_handler" do
    it ": handles errors correctly and displays them to screen" do
      expect(ApplicationView.error_handler("Cannot access API", 400)).to match("Sorry! We have encountered an error: Cannot Access the API the error code is 400")
      expect(ApplicationView.error_handler("Authentication failed, please check your credentials", 401)).to match("Sorry! We have encountered an error: Authentication Failed, please check your credentials")
      expect(ApplicationController.error_handler(nil, nil)).to match("Sorry! An unknown error has occured. Please restart the program.")
    end
  end
end